"""
Shapefile utilities for OLADPP.
Handles reading and writing shapefiles with proper validation and error handling.
"""
import os
from pathlib import Path
from typing import Set, Optional, Dict, Any
import geopandas as gpd
from ..utils.logging import log_debug, log_error, log_warning, ensure_path_exists
import traceback
...
def _validate_geometry_types(geom_types: Set[str], layer_name: str) -> Optional[str]:
    """Validate geometry types in a GeoDataFrame.

    Args:
        geom_types: Set of geometry types found
        layer_name: Name of the layer being processed

    Returns:
        Valid geometry type or None if invalid
    """
    if not geom_types:
        return None
...
def read_shapefile(file_path: str) -> Optional[gpd.GeoDataFrame]:
    ...
    Returns:
        GeoDataFrame or None if reading fails
    """
    try:
        ...
        log_debug(f"Reading shapefile: {file_path}")
        ...
        if invalid_geoms.any():
            ...
            log_warning(f"Found {invalid_count} invalid geometries in {file_path}")

        return gdf

    except Exception as e:
        log_error(f"Error reading shapefile {file_path}: {str(e)}")
...
def _delete_existing_shapefile(directory: str, layer_name: str) -> None:
    """Delete all existing shapefile components for the given layer.

    Args:
        directory: Directory containing the shapefile
        layer_name: Name of the layer (without extension)
    """
