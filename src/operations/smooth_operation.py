...
def create_smooth_layer(all_layers, project_settings, crs, layer_name, operation):
        ...
        for layer_info in source_layers:
            ...
            layer_geometry = _get_filtered_geometry(all_layers, project_settings, crs, source_layer_name, values, column_name)
        ...
        if combined_geometry is not None:
            ...
            smoothed_geometry = smooth_geometry(all_layers, project_settings, crs, combined_geometry, strength)
            if make_valid:
                smoothed_geometry = make_valid_geometry(smoothed_geometry)
            cleaned_geometry = smoothed_geometry
            all_layers[layer_name] = ensure_geodataframe(all_layers, project_settings, crs, layer_name, gpd.GeoDataFrame(geometry=[cleaned_geometry], crs=crs))
