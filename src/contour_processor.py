...
def process_contour(operation, geltungsbereich, buffer_distance, project_crs):
    ...
    if atom_file is None:
        try:
            ...
            log_debug("Successfully downloaded and cached ATOM feed")
    ...
    try:
        ...
        log_debug("Successfully parsed ATOM feed")
    except ET.ParseError as e:
        log_error(f"Failed to parse ATOM feed: {str(e)}")
        return None

    log_debug("Searching for DGM data download links")
    ...
    log_debug(f"Found {len(download_links)} potential DGM data files")

    log_debug(f"Project CRS: {project_crs}")
    log_debug(f"Geltungsbereich bounds: {geltungsbereich.bounds}")
    
    geltungsbereich_buffered = geltungsbereich.buffer(buffer_distance)
    log_debug(f"Buffered geltungsbereich bounds: {geltungsbereich_buffered.bounds}")
        ...
        if href and href.endswith('_isoli.zip'):
            ...
            if bbox_str:
                ...
                if geltungsbereich_buffered.intersects(tile_box):
                    ...
                    log_debug(f"Found intersecting isoli tile: {bbox} -> {transformed_bbox}")

    log_debug(f"Found {len(relevant_links)} relevant isoli data files")
    ...
    for link in relevant_links:
        ...
        if cached_zip is None:
            try:
                ...
                log_debug(f"Downloaded and cached: {cached_zip}")
        ...
        if not os.path.exists(extracted_dir):
            ...
            log_debug(f"Extracted contents to: {extracted_dir}")
    ...
    log_debug("Reading and merging shape files")
    gdf_list = []
    ...
    log_debug(f"Successfully merged shape data. Total features: {len(merged_gdf)}")

    log_debug(f"Clipping contours to geltungsbereich with buffer {buffer_distance}")
    clipped_gdf = merged_gdf.clip(geltungsbereich_buffered)
    log_debug(f"Clipped GeoDataFrame contains {len(clipped_gdf)} features")

    # Ensure all geometries are LineStrings
    clipped_gdf = clipped_gdf[clipped_gdf.geometry.type == 'LineString']
    log_debug(f"Final GeoDataFrame contains {len(clipped_gdf)} LineString features")

    log_debug("Contour processing completed successfully")
