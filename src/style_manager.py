...
class StyleManager:
    ...
    def process_entity_style(self, layer_name, layer_config):
        """Process entity-level style properties"""
        entity_properties = {}
        ...
        if 'style' in layer_config:
            ...
            if not warning_generated and style is not None:
                # Extract entity settings from the style
                entity_style = style.get('entity', {}) if isinstance(style, dict) else {}
        ...
        # Also check for direct entity properties in layer config
        for key in ['close', 'linetypeScale', 'linetypeGeneration']:
            if key in layer_config:
                entity_properties[key] = layer_config[key]
