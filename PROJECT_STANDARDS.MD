...
- **External Library Understanding (NEW):** When working with integrated external libraries that include local documentation (e.g., `ezdxf` in this project), the AI **SHOULD** explore this local documentation first. For instance, `ezdxf/docs/source/reference.rst` serves as a valuable master table of contents for its documentation, aiding in efficient API understanding and usage verification.
